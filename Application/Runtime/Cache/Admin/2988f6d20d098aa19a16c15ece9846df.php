        ...
        <div class="col-sm-11">
            ...
            <section class="user-timeline-stories">
                ...
                <article class="timeline-story">
                    ...
                    <!-- User info -->
                    <header>
                        <h3><i class="fa-rss"></i><?php echo ($info["title"]); ?></h3>
                        <a class="user-img">
                            <?php if(empty($info['author_info']['icon'])): ?><img src="/Public/Admin/images/user-4.png" alt="user-img" class="img-responsive img-circle" />
                            <?php else: ?>
                                <img src="<?php echo ($info["author_info"]["icon"]); ?>" alt="user-img" class="img-responsive img-circle" /><?php endif; ?>
                        </a>
                        <div class="user-details">
                            <?php if(empty($info['author'])): ?><span><strong><?php echo (session('username')); ?></strong></span>
                                <!-- <input type="hidden" id="info_uid" name="uid" value="<?php echo ($info_vo['uid']); ?>"> -->
                            <?php else: ?>
                                <span><strong><?php echo ($info["author_info"]["uname"]); ?></strong></span>
                                <input type="hidden" id="info_uid" name="uid" value="<?php echo ($info["author_info['uid']"]); ?>"><?php endif; ?>
                            <time><?php echo (date('Y-m-d H:i',$info["send_time"])); if($info['is_money'] == 1): ?><!--<span style="padding-left: 15px;color: #ff3b2f;">获得<?php echo ($info['author_amount']); ?>元求助奖励红包</span>--><?php endif; ?></time>
                        </div>
                    </header>
                    <div class="story-content">
                        <?php if(!empty($info['title_img'])): ?><div class="col-header-options" align="center">
                                <!--<a class="base-padding">-->
                                    <!--<img style="width: auto;height: 200px;" src="<?php echo ($info["title_img"]); ?>" alt="album-image" class="img-responsive" title="标题图片"/>-->
                                <!--</a>-->
                            </div><?php endif; ?>
                        ...
                        <div class="mail-single-body">
                            ...
                            <div class="form-group">
                                <!--<div class="form-control wysihtml5" data-stylesheet-url="/Public/Admin/js/wysihtml5/lib/css/wysiwyg-color.css" name="sample_wysiwyg" id="sample_wysiwyg"></div>-->
                                <pre><?php echo ($content); ?></pre>
                            </div>
                        </div>

                        <ul class="list-unstyled list-inline">
                            <?php if(!empty($info['imgs'])): $_result=explode(',',$info['imgs']);if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo_img): $mod = ($i % 2 );++$i;?><li class="col-2">
                                        <a>
                                            <img style="width: 200px;height: 150px;" src="<?php echo (img_path($vo_img)); ?>" alt="album-image" class="img-responsive" />
                                        </a>
                                    </li><?php endforeach; endif; else: echo "" ;endif; endif; ?>
                        </ul>

                        <ul class="list-unstyled list-inline">
                            <?php if(!empty($info["tag_city"])): $_result=explode(',',$info['tag_city']);if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$dat_cit): $mod = ($i % 2 );++$i;?><li><span class="label label-success">#<?php echo ($dat_cit); ?>#</span></li><?php endforeach; endif; else: echo "" ;endif; endif; ?>
                            <?php $_result=explode(',',$info['tag_major']);if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$dat_vo): $mod = ($i % 2 );++$i; if(is_array($major)): $i = 0; $__LIST__ = $major;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$major_vo): $mod = ($i % 2 );++$i; if($major_vo['id'] == $dat_vo): ?><li><span class="label label-warning">#<?php echo ($major_vo["delimiter"]); echo ($major_vo["major_name"]); ?>#</span></li><?php endif; endforeach; endif; else: echo "" ;endif; endforeach; endif; else: echo "" ;endif; ?>
                        </ul>
                        ...
                        <div class="story-options-links">
                            <a>
                                ...
                                浏览
                                <span>(<?php echo ($info["views"]); ?>)</span>
                            </a>

                            <a>
                                <i class="linecons-heart"></i>
                                收藏
                                <span>(<?php echo ($info["favorite_sums"]["counts"]); ?>)</span>
                            </a>

                            <a>
                                <i class="linecons-comment"></i>
                                评论
                                <span>(<?php echo ($info["comment_sums"]["counts"]); ?>)</span>
                            </a>
                            <!-- 发红包按钮 -->
                            <?php if($info['post_amount'] == null): ?><input id="btn-author" type="button" value="发红包" class="btn btn-warning" uid="<?php echo ($info["author"]); ?>" res_id="<?php echo ($info["id"]); ?>" onclick="postAuthorMoney(this)">
                            <?php else: ?>
                                <button id="btn-author-money" type="button" class="btn btn-warning" disabled>已获得官方<?php echo ($info["post_amount"]); ?>元指定奖励</button><?php endif; ?>
                        </div>

                        <!-- Story Comments -->
                        <ul class="list-unstyled story-comments">
                            <?php if(is_array($info['comment_info'])): foreach($info['comment_info'] as $key=>$info_vo): ?><li>
                                    <div class="story-comment">
                                        <a class="comment-user-img">
                                            <?php if(empty(user_info($info_vo['uid'],'icon'))){ echo '<img src="/Public/Admin/images/default_head.png" alt="user-img" class="img-circle img-responsive" />'; }else { echo '<img src="'.user_info($info_vo['uid'],'icon').'" alt="user-img" class="img-circle img-responsive" />'; } ?>
                                        </a>
                                        <div class="story-comment-content">
                                            <span class="story-comment-user-name">
                                                <?php echo (user_info($info_vo['uid'],'uname')); ?>
                                                <time><?php echo (date('Y-m-d H:i',$info_vo["time"])); ?></time>
                                                <?php if($info_vo['amount'] != null): ?><span style="padding-left: 15px;color: #ff3b2f;">获得<?php echo ($info_vo['amount']); ?>元回复奖励红包</span><?php endif; ?>
                                            </span>
                                            <p><?php echo ($info_vo["comm_content"]); ?></p>
                                        </div>
                                        <!-- Story Options Links -->
                                        <div class="story-options-links">
                                            <a>
                                                ...
                                                点赞
                                                <span>(<?php echo ($info_vo["likes"]); ?>)</span>
                                            </a>

                                            <a>
                                                <i class="el-thumbs-down"></i>
                                                踩
                                                <span>(<?php echo ($info_vo["dislikes"]); ?>)</span>
                                            </a>

                                            <?php if($info['type'] == 2): if($info['tbd_id'] != $info_vo['id']): ?><a href="<?php echo U('Content/del_comment',array('t'=>lx_encrypt(1),'cid'=>lx_encrypt($info_vo['id'])));?>" onclick="return del_confirm()">
                                                        <i class="el-trash"></i>
                                                        删除
                                                    </a><?php endif; ?>
                                                <?php if(($info['author'] == 0) AND ($info['tbd_id'] == 0)): ?><a href="<?php echo U('Content/adoptHelp',array('cid'=>$info_vo['id'],'rid'=>$info['id']));?>">
                                                        <i class="linecons-star"></i>
                                                        采纳回答
                                                    </a>
                                                <?php elseif($info['tbd_id'] == $info_vo['id']): ?>
                                                    <span class="btn btn-pink btn-icon">已采纳</span><?php endif; ?>
                                                <?php else: ?>
                                                <a href="<?php echo U('Content/del_comment',array('t'=>lx_encrypt(1),'cid'=>lx_encrypt($info_vo['id'])));?>" onclick="return del_confirm()"><i class="el-trash"></i>delete
                                                </a><?php endif; ?>
                                        </div>
                                    </div>
                                </li>
...
<div id="content" class="hide">
    <form id="form" class="form-horizontal" method="post" action="<?php echo U('Money/postMoney');?>">
        <div class="row">
...
<script type="text/javascript" src="/Public/bui/js/config-min.js"></script>
        ...
        var dialog = new Overlay.Dialog({
            ...
            success:function() {
                ...
                if(r) {
                    ...
                    var pUrl = "<?php echo U('Admin/Money/postAuthorMoney');?>";
