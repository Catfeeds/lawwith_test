            ...
            <div class="panel panel-default">
                <div class="panel-body">
                    <div class="search-form input-group col-xs-3 pull-right">
                        <input type="text" name="nickname" value="<?php echo I('nickname');?>" class="form-control search-input" placeholder="请输入律所名称&hellip;">
                        <a class="input-group-addon" href="javascript:;" id="search" url="<?php echo U('index');?>">
                            <i class="linecons-search"></i>
                        </a>
                    </div>
                    <form method="post" enctype="multipart/form-data" action="<?php echo U('Laws/subimport');?>">
...
//                                            alert(vals);
                                            ...
                                            var pUrl = "<?php echo U('Laws/delLaws');?>";
                                            $.post(pUrl, data, function(data){
                                                if(data.state == 1){
                                                    alert(data.msg);
                                                    location.href = "<?php echo U('Laws/index');?>";
                                                }else{
                                                    alert("操作失败");
                                                }
                                            }, 'json');
                        ...
                        <table cellspacing="0" class="table table-small-font table-bordered table-striped" id="law_list">
                            ...
                            <?php if(is_array($data)): foreach($data as $key=>$vo): ?><tr>
                                    <td>
                                        <input name="law_id" type="checkbox" class="cbr" value="<?php echo ($vo["id"]); ?>">
                                    </td>
                                    <th><?php echo ($key+1); ?></th>
                                    <th>
                                        <?php if(!empty($vo['logo'])): ?><img src="<?php echo ($vo["logo"]); ?>" alt="user-image" class="img-circle img-inline userpic-32" width="28" />
                                        <?php else: ?>
                                            暂无<?php endif; ?>
                                    </th>
                                    <th><?php echo ($vo["law_name"]); ?></th>
                                    <td>
                                        <?php if(empty($vo['uadmin'])): ?>暂无
                                            <?php else: ?>
                                            <a href=""><?php echo ($vo["uname"]); ?></a><?php endif; ?>
                                    </td>
                                    <td>
                                        <?php if(!empty($vo['province'])): echo ($vo["province"]); ?> -<?php endif; ?>
                                        <?php if(!empty($vo['city'])): echo ($vo["city"]); ?> -<?php endif; ?>
                                        <?php if(!empty($vo['town'])): echo ($vo["town"]); endif; ?>
                                        <?php if(!empty($vo['address'])): ?>- <?php echo ($vo["address"]); endif; ?>
                                    </td>
                                    <td>
                                        <?php if(empty($vo['phone'])): ?>暂无
                                            <?php else: ?>
                                            <a href=""><?php echo ($vo["phone"]); ?></a><?php endif; ?>
                                    </td>
                                    <td>
                                        <!--<?php $_result=explode(',',$vo['majors']);if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vx): $mod = ($i % 2 );++$i;?>-->
                                            <!--<?php if(is_array($major_list)): foreach($major_list as $key=>$mvo): ?>-->
                                                <!--<?php if($mvo['id'] == $vx): ?>【<?php echo ($mvo["major_name"]); ?>】<?php endif; ?>-->
                                            <!--<?php endforeach; endif; ?>-->
                                        <!--<?php endforeach; endif; else: echo "" ;endif; ?>-->【<?php echo ($mvo["major_name"]); ?>】
                                    </td>
                                    ...
                                    <td>
                                        <?php if($vo['status'] == 1): ?><a href="<?php echo U('Laws/changeStatus',array('status'=>lx_encrypt(0),'lid'=>lx_encrypt($vo['id'])));?>" onclick="return del_confirm()" class="btn btn-danger btn-sm btn-icon icon-left">删除</a>
                                            <?php else: ?>
                                            <a href="<?php echo U('Laws/changeStatus',array('status'=>lx_encrypt(1),'lid'=>lx_encrypt($vo['id'])));?>" class="btn btn-secondary btn-sm btn-icon icon-left">恢复</a><?php endif; ?>
                                        <?php if($vo['is_push'] == 1): ?><a href="<?php echo U('Laws/cancelPush',array('is_push'=>lx_encrypt(0),'mid'=>lx_encrypt($vo['id'])));?>" class="btn btn-pink btn-sm btn-icon icon-left">取消banner推荐</a>
                                            <?php else: ?>
                                            <a href="<?php echo U('Push/addPush',array('id'=>lx_encrypt($vo['id']),'t'=>lx_encrypt(5)));?>" class="btn btn-turquoise btn-sm btn-icon icon-left">banner推荐</a><?php endif; ?>
                                        <a href="<?php echo U('Laws/editLaws',array('lid' => lx_encrypt($vo['id'])));?>" class="btn btn-info btn-sm btn-icon icon-left">
                                            <li class="fa-pencil"></li>
                                        </a>
                                        <!--<a href="<?php echo U('Laws/delLaws',array('lid'=>$vo['id']));?>" onclick="return del_confirm()" class="btn btn-danger btn-sm btn-icon icon-left">-->
                    ...
                    <script type="text/javascript">
                        ...
                        function addLaws(){
                            window.location.href = "<?php echo U('Laws/addLaws');?>";
                        }

//                        function import_excel(){
//                            window.location.href = "<?php echo U('Laws/subimport');?>";
//                        }

                        function export_excel(){
                            window.location.href = "<?php echo U('Laws/subexport');?>"
                        }
                    </script>
