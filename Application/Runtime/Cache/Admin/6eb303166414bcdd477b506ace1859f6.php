...
<div class="panel panel-default">
    <div class="panel-body">
        <div class="search-form input-group col-xs-3 pull-right">
            ...
            <a class="input-group-addon" href="javascript:;" id="search" url="<?php echo U('redpacket');?>">
        ...
        <table class="table table-bordered table-striped" id="del_data">
            ...
            <?php if(is_array($data)): foreach($data as $key=>$vo): ?><tr>
                    ...
                    <td>
                        <?php if(empty($vo["user_name"])): ?>该用户还未设置用户名
                        <?php else: ?>
                            <a href="<?php echo U('User/audit_identity',array('uid'=>lx_encrypt($vo['uid'])));?>"><strong><?php echo ($vo["user_name"]); ?></strong></a><?php endif; ?>
                    </td>
                    <td><strong><?php echo ($vo["money"]); ?></strong></td>
                    <td>
                        <?php switch($vo["type"]): case "1": ?>充值<?php break;?>
                            <?php case "2": ?>提现<?php break;?>
                            <?php case "3": ?>红包<?php break;?>
                            <?php case "4": ?>付费求助<?php break;?>
                            <?php case "5": ?>打赏<?php break; endswitch;?>
                    </td>
                    <td>
                        <?php switch($vo["pay_type"]): case "1": ?>支付宝<?php break;?>
                            <?php case "2": ?>微信<?php break;?>
                            <?php case "3": ?>银行卡<?php break;?>
                            <?php case "4": ?>余额<?php break;?>
                            <?php case "0": ?>待定<?php break; endswitch;?>
                    </td>
                    <td>
                        <?php switch($vo["trade_type"]): case "1": ?>收入<?php break;?>
                            <?php case "2": ?>支出<?php break; endswitch;?>
                    </td>
                    <td><?php echo (date('Y-m-d H:i',$vo["pay_time"])); ?></td>
                    <td><?php echo (msubstr($vo["trade_title"],0,30,"utf-8",true)); ?></td>
...
<script>
    ...
    function subexport(){
        window.location.href = '<?php echo U("Money/exportRedpack");?>';
    }
</script>
