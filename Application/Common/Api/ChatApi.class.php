...
class ChatApi {
     ...
     */
    public function __construct($AppKey = '2efb9a02309e00ec9153324af405dea8',$AppSecret = '76748ca07f92',$RequestType='curl'){
        $this->AppKey    = $AppKey;
...
//        print_r($result);
    ...
    /**
     * 消息功能-批量发送普通消息
     * @param  $fromAccid       [发送者accid，用户帐号，最大32字节，必须保证一个APP内唯一]
     * @param  $toAccids        [["aaa","bbb"]（JSONArray对应的accid，如果解析出错，会报414错误），限500人]
     * @param  $type        [0 表示文本消息,1 表示图片，2 表示语音，3 表示视频，4 表示地理位置信息，6 表示文件，100 自定义消息类型]
     * @param  $body       [请参考下方消息示例说明中对应消息的body字段。最大长度5000字节，为一个json字段。]
     * @param  $option       [发消息时特殊指定的行为选项,Json格式，可用于指定消息的漫游，存云端历史，发送方多端同步，推送，消息抄送等特殊行为;option中字段不填时表示默认值]
     * @param  $pushcontent      [推送内容，发送消息（文本消息除外，type=0），option选项中允许推送（push=true），此字段可以指定推送内容。 最长200字节]
     * @param  $payload         [ios 推送对应的payload,必须是JSON,不能超过2k字节]
     * @param  $ext         [开发者扩展字段，长度限制1024字节]
     */
    public function sendMsgs($fromAccid,$toAccids,$type,$body,$option='{"push":false,"roam":true,"history":false,"sendersync":true, "route":false}',$pushcontent='',$payload,$ext=''){
     ...
     * 1.自定义系统通知区别于普通消息，方便开发者进行业务逻辑的通知。
     * 2.目前支持两种类型：点对点类型。
     * @param  $fromAccid       [发送者accid，用户帐号，最大32字节，APP内唯一]
     ...
     */
    public function sendAttachMsgs($fromAccid,$toAccids,$attach,$pushcontent='',$payload='{}',$sound='',$save=2,$option){
     ...
     * @param  $cid       [频道ID，32位字符串]
     */
    public function channelPause($cid){
     ...
     * @param  $cid       [频道ID，32位字符串]
     */
    public function channelResume($cid){
     ...
     * 聊天室消息抄送
     * @param $attach       [消息内容]
     * @param $eventType    [值为4，表示是聊天室消息]
     * @param $fromAccount  [消息发送者的账号]
     * @param $msgTimestamp [消息发送的时间戳]
     * @param $msgType      [消息类型]
     * @param $msgidClient  [客户端生成的消息id]
     * @param $roomId       [消息所属的聊天室id]
     * @param $ext          [第三方扩展字段]
     * @param $fromAvator   [发送者的头像]
     * @param $fromClientType   [客户端类型]
     * @param $fromExt      [发送者身份的扩展字段]
     * @param $fromNick     [发送方昵称]
     * @param $resendFlag   [重发标记]
     * @param $roleInfoTimetag  [消息发送者用户名片的最后更新时间]
     *
     */
    public function copyReceiveMsg($attach,$eventType=4,$fromAccount,$msgTimestamp,$msgType='TEXT',$msgidClient,$roomId,$ext='',$fromAvator='',$fromClientType,$fromExt='',$fromNick='',$resendFlag=0,$roleInfoTimetag=0)
    {
        $url = 'http://yunxinservice.com.cn/receiveMsg.action';
        $data = array(
            'attach' => $attach,
            'eventType' => $eventType,
    ...
    /**
     * 更新聊天室信息
     * @param  $roomid              必须 [聊天室id]
     * @param  $name                必须 [聊天室名称，长度限制128个字符]
     * @param  $announcement        [公告，长度限制4096个字符]
     * @param  $broadcasturl        [直播地址，长度限制1024个字符]
     * @param  $ext                 [扩展字段，长度限制4096个字符]
     * @param  $needNotify          [true或false,是否需要发送更新通知事件，默认true]
     * @param  $notifyExt           [通知事件扩展字段，长度限制2048]
     */
    public function chatRoomUpdate($roomid,$name,$announcement,$broadcasturl)
    {
     ...
     * @param  $opt                 [操作：1: 设置为管理员，operator必须是创建者 2:设置普通等级用户，operator必须是创建者或管理员 -1:设为黑名单用户，operator必须是创建者或管理员 -2:设为禁言用户，operator必须是创建者或管理员]
     * @param  $optvalue            [true或false，true:设置；false:取消设置]
     * @param  $notifyExt           [通知扩展字段，长度限制2048，请使用json格式]
     */
    public function chatRoomSetAccount($roomid,$operator,$target,$opt,$optvalue,$notifyExt)
    {
     ...
     * @param  $roleExt             [机器人信息扩展字段，请使用json格式，长度4096字符]
     * @param  $notifyExt           [机器人进入聊天室通知的扩展字段，请使用json格式，长度2048字符]
     */
     ...
     */
    public function getrequestAddr($roomid,$accid,$clienttype=1)
    {
