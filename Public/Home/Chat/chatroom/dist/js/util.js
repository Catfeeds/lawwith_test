var util ={
	...
	buildEmoji:function(text) {
		...
		for (var j = 0, len = matches.length; j < len; ++j) {
			if(emoji[matches[j]]){
				text = text.replace(matches[j], '<img class="emoji" src="/Public/Home/Chat/chatroom/images/emoji/' + emoji[matches[j]].file + '" />');
			}		
		}
		return text;
	},
        ...
        switch (data.type){
            ...
            case 'custom':
                if (!data.content) {
                    ...
                    if(content.type===1){
                        text = '<img src="/Public/Home/Chat/chatroom/images/play-'+content.data.value+'.png" onload="page.scrollToBottom()">';
                    }   
                }
                break;
            default:
