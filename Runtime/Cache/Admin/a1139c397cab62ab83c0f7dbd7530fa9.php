    ...
    <table class="table table-bordered table-striped" id="train_list">
        ...
        <tbody class="middle-align">
            <?php if(is_array($data)): foreach($data as $key=>$vo): ?><tr>
                    <td><?php if(!empty($vo["title_img"])): ?><img src="<?php echo ($vo["title_img"]); ?>" width="150px;" height="90px;"><?php else: ?>暂无<?php endif; ?></td>
                    <td><span><?php echo (msubstr($vo["title"],0,30,"utf-8",true)); ?></span></td>
                    <td><span><?php echo ($vo["cate_name"]); ?></span></td>
                    <td><span><?php echo (date('Y-m-d',$vo["create_at"])); ?></span></td>
                    <td>
                        <?php if(is_array($major_list)): foreach($major_list as $key=>$mvo): if($mvo['id'] == $vo['tags']): ?>【<?php echo ($mvo["major_name"]); ?>】<?php endif; endforeach; endif; ?>
                    </td>
                    <td>
                    <span>
                        <?php if($vo['is_push'] == 1): ?><a href="<?php echo U('Video/cancelPush',array('is_push'=>lx_encrypt(0),'id'=>lx_encrypt($vo['id']),'mark'=>lx_encrypt(1)));?>" style="color: red">取消首页</a>
                            <?php else: ?>
                            <a href="<?php echo U('Push/addPush',array('id'=>lx_encrypt($vo['id']),'t'=>lx_encrypt(4)));?>" class="">首页推荐</a><?php endif; ?>&nbsp;&nbsp;
                        <?php if($vo['recom_push'] == 1): ?><a href="<?php echo U('Video/recom_push',array('status'=>lx_encrypt(0),'mid'=>lx_encrypt($vo['id'])));?>" style="color: red">取消精选</a>
                        <?php else: ?>
                            <a href="<?php echo U('Video/recom_push',array('mid'=>lx_encrypt($vo['id']),'status'=>lx_encrypt(1)));?>" class="">精选推荐</a><?php endif; ?>
                        &nbsp;&nbsp;
                        <a href="<?php echo U('Video/update',array('vid'=>$vo['id']));?>">编辑</a>
                        &nbsp;&nbsp;
                        <a href="<?php echo U('Video/delete',array('vid'=>$vo['id']));?>" onclick="return del_confirm()">删除</a>
                        &nbsp;&nbsp;
                        <a href="<?php echo U('Video/view',array('vid'=>$vo['id']));?>">查看</a>
                    </span>
                    </td>
...
<script type="text/javascript">
    function videoUpload(){
        window.location.href = "<?php echo U('Video/create');?>";
    }
</script>
<script type="text/javascript">
    //搜索功能
        ...
        $("#del_data").click(function() {
            ...
            if (r)
            {
                ...
                if (vals == ''){
                    ...
                    var pUrl = "<?php echo U('Video/delTrains');?>";
