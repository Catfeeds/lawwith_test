    ...
    <table class="table table-bordered table-striped" id="train_list">
        ...
        <tbody class="middle-align">
            <?php if(is_array($data)): foreach($data as $key=>$vo): ?><tr>
                    <td><span><?php echo ($vo["cate_id"]); ?></span></td>
                    <td><span><?php echo ($vo["cate_name"]); ?></span></td>
                    <td><span><?php echo ($vo["sort"]); ?></span></td>
                    <td><span><?php echo ($vo["status"]); ?></span></td>
                    <td><span><?php echo (date('Y-m-d',$vo["create_time"])); ?></span></td>
                    <td><span><?php echo ($vo["author"]); ?></span></td>
                    <td>
                    <span>
                        <?php if($vo['status'] == 1): ?><a href="<?php echo U('Video/updateCategoryStatus',array('cate_id'=>$vo['cate_id'],'status'=>$vo['status']));?>">隐藏</a>
                        <?php else: ?>
                            <a href="<?php echo U('Video/updateCategoryStatus',array('cate_id'=>$vo['cate_id'],'status'=>$vo['status']));?>" style="color: red">显示</a><?php endif; ?>
                        &nbsp;&nbsp;
                        <a href="<?php echo U('Video/categoryUpdate',array('cate_id'=>$vo['cate_id']));?>">编辑</a>
                        &nbsp;&nbsp;
                        <a href="javascript:categoryDelete(<?php echo ($vo['cate_id']); ?>);">删除</a>
                    </span>
                    </td>
...
<script type="text/javascript">
    function create() {
        window.location.href = "<?php echo U('Video/categoryCreate');?>";
    }

    function categoryDelete(cate_id) {
        if(confirm('该删除不可恢复,确定删除吗?')) {
            $.ajax({
                async: false, type: 'post', url: "<?php echo U('Video/categoryDelete');?>", data: {cate_id: cate_id},
                error: function () {
                    alert('删除失败^,程序异常');
                },
                success: function (response) {
                    ...
                    if (JSON.parse(response).msg) {
                        window.location.href = "<?php echo U('Video/category');?>";
                    } else {
